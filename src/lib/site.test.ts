...
test("getSiteUrl uses VERCEL_URL when explicit site URL is not set", () => {
  const prevPublic = process.env.NEXT_PUBLIC_SITE_URL;
  const prevSite = process.env.SITE_URL;
  const prevVercel = process.env.VERCEL_URL;
  const prevNodeEnv = process.env.NODE_ENV;
  ...
  delete process.env.SITE_URL;
  process.env.VERCEL_URL = "genpasswd.vercel.app";
  process.env.NODE_ENV = "production";

  try {
    assert.equal(getSiteUrl().origin, "https://genpasswd.vercel.app");
    assert.equal(absoluteUrl("/"), "https://genpasswd.vercel.app/");
  } finally {
    if (prevPublic === undefined) delete process.env.NEXT_PUBLIC_SITE_URL;
    else process.env.NEXT_PUBLIC_SITE_URL = prevPublic;
    if (prevSite === undefined) delete process.env.SITE_URL;
...
test("getSiteUrl falls back to default prod URL in production", () => {
  const prevPublic = process.env.NEXT_PUBLIC_SITE_URL;
  const prevSite = process.env.SITE_URL;
  const prevVercel = process.env.VERCEL_URL;
  const prevNodeEnv = process.env.NODE_ENV;
